...
fn decode_wait() {
    // wait const(1)
    let code = vec![0x02, 0b1100_0000, 0x1];
...
#[test]
fn decode_set() {
    // set b1 ret(0) const(7)
    let code = vec![0x03, 0b1100_0000, 0x7, 0x0];
    let set = decode(code).unwrap();
    assert_eq!(set, Op::Set(RefRet::Return(0), Value::Const(7), OpSize::B1));

    // set b2 ref(1) ref(256)
    let code = vec![0x03, 0b0001_0100, 0x0, 0x1, 0x1];
    let set = decode(code).unwrap();
    assert_eq!(set, Op::Set(RefRet::Ref(1), Value::Ref(256), OpSize::B2));

    // set b2 ret(5) const(257)
    let code = vec![0x03, 0b1101_1000, 0x1, 0x1, 0x0, 0x0, 0x5];
    let set = decode(code).unwrap();
...
fn decode_sub() {
    // sub b4 1 ref(7)
    let code = vec![0x05, 0b0010_0100, 0x7, 0x0, 0x1];
    let sub = decode(code).unwrap();
    assert_eq!(sub, Op::Sub(Ref(1), Value::Ref(7), None, OpSize::B4));

    // sub b8 1 const(7)
    let code = vec![0x05, 0b0111_0100, 0x7, 0x0, 0x1];
    let sub = decode(code).unwrap();
